...
func (_m *V2DBClient) GetFinalityProviderLogos(ctx context.Context) ([]v2dbmodel.FinalityProviderLogo, error) {
	...
	if len(ret) == 0 {
		panic("no return value specified for GetFinalityProviderLogos")
	}

	var r0 []v2dbmodel.FinalityProviderLogo
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]v2dbmodel.FinalityProviderLogo, error)); ok {
		return rf(ctx)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []v2dbmodel.FinalityProviderLogo); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
