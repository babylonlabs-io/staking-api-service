...
// MarkV1DelegationAsTransitioned provides a mock function with given fields: ctx, stakingTxHashHex
func (_m *V2DBClient) MarkV1DelegationAsTransitioned(ctx context.Context, stakingTxHashHex string) error {
	ret := _m.Called(ctx, stakingTxHashHex)

	if len(ret) == 0 {
		panic("no return value specified for MarkV1DelegationAsTransitioned")
	}
